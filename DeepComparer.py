...
def main() -> None:
    ...
    cap:cv2.VideoCapture = cv2.VideoCapture(stream)
    if not cap.isOpened():
        return
    
    count:int = 0
    out:cv2.VideoWriter = cv2.VideoWriter('output.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (1280, 720))
    ...
    while True:
        ...
        display_frame:cvImage = cv2.resize(frame.copy(), (1280, 720))
        
        for class_name, probability in results.items():
            if probability > 90:
                cv2.putText(display_frame, f'{class_name}:{probability:.2f}%', (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        
        cv2.imshow('cctv', display_frame)
